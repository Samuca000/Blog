...
def post_list(request, tag_slug=None):
    ...
    try:
       posts = paginator.page(page)
        ...
        #se a página  nao for um int, exibe a first page
        posts = paginator.page(1)
    except EmptyPage:
        #If a page was fora of the intervalo,
        #exibe a última page of results
...
def post_detail(request, year, month, day, post):
    ...
    #Lista de todos comentários ativos para esta postagem
    comments = post.comments.filter(active=True)
    
    new_comment = None
    if request.method == 'POST':
        #Um comentário foi passado
        comment_form = CommentForm(data=request.POST)
        if comment_form.is_valid():
            #Criao objeto comment,mas não
...
def post_share(request, post_id):
    #Obetenha a postagem com base no id
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            #Campos do form que passaram pela validação
